...
// Asset stages — matches the client's Excel exactly:
//   Names · Artist Name · Status · Start Date · End Date
// No Time Used column on assets — that lives on Shot Wise stages per the spec.
...
const SHOT_COLUMNS: ColumnConfig[] = [
  ...
  { key: 'startDate',        label: 'Start Date',           type: 'date',        width: 120 },
  { key: 'endDate',          label: 'End Date',             type: 'date',        width: 120 },
  { key: 'timeConsumed',     label: 'Time Used',            type: 'number',      width: 120 },
]
...
/** subStageId → parent stage slug (e.g. 'animation-animation' → 'animation') */
export const SUB_STAGE_TO_STAGE_SLUG: Record<string, string> = Object.fromEntries(
  STAGE_CONFIGS.flatMap(s => s.subStages.map(ss => [ss.id, s.slug]))
)

/**
 * Auto-mirror rules — when a task is created in the key sub-stage,
 * a linked copy (same name + artist, all other fields at defaults)
 * is automatically created in each of the listed target sub-stages.
 *
 * This eliminates redundant data entry across pipeline stages that
 * share the same assets (Character, Props, BG).
 */
export const MIRROR_RULES: Record<string, string[]> = {
  // Character in Modelling → Blendshapes + all downstream character stages
  'modelling-character': [
  ...
  // Props in Modelling → all downstream props stages
  'modelling-props': [
    'unwrapping-props',
    'texturing-props',
    'rigging-props',
  ],
  // BG in Modelling → all downstream BG stages
  'modelling-bg': [
    'unwrapping-bg',
    'texturing-bg',
    'rigging-bg',
