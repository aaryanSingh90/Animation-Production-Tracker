      ...
      {activeStages.map((stage) => {
        ...
        return (
          <div
            ...
            style={{ backgroundColor: STATUS_COLORS[normalizedStatus] || "#6B7280" }}
            title={`${getStageDisplayName(stage)} · ${getStatusOptionLabel(normalizedStatus)}${stage.deadline ? ` · ${formatDate(stage.deadline)}` : ""}${commentCount ? ` · ${commentCount} comment${commentCount > 1 ? "s" : ""}` : ""}`}
          >
            {commentCount > 0 && (
